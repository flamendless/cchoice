...
-- name: AdminCountProductsForListing :one
...
-- name: AdminGetProductsForListingPaginated :many
SELECT
	tbl_products.id,
	tbl_products.name,
	tbl_products.serial,
...
-- name: AdminGetProductsForExport :many
SELECT
	tbl_products.id,
	tbl_products.name,
	tbl_products.serial,
...
WHERE
	(@search_brand IS NULL OR @search_brand = '' OR LOWER(tbl_brands.name) = LOWER(@search_brand))
	AND (@status IS NULL OR @status = '' OR tbl_products.status = @status)
;

-- name: AdminCountProductsForExport :one
SELECT COUNT(*) AS count
FROM tbl_products
INNER JOIN tbl_brands ON tbl_brands.id = tbl_products.brand_id
WHERE
...
-- name: GetProductsBySearchQueryPaginated :many
SELECT
	tbl_products.id,
	tbl_products.serial,
	tbl_products.slug,
...
-- name: GetRelatedProductsForSearch :many
SELECT
	tbl_products.id,
	tbl_products.serial,
	tbl_products.slug,
...
LEFT JOIN tbl_products_categories ON tbl_products_categories.product_id = tbl_products.id
LEFT JOIN tbl_product_categories AS pc ON pc.id = tbl_products_categories.category_id
WHERE tbl_products.slug = '' OR tbl_products.slug IS NULL;
